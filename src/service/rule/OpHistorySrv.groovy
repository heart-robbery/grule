...
class OpHistorySrv extends ServerTpl {
    ...
    void enHistory(IEntity entity, String operator) {
        def data = Utils.toMapper(entity).ignore("metaClass")
            ...
            .build()
        repo.saveOrUpdate(
            new OpHistory(tbName: repo.tbName(entity.class).replace("`", ""), operator: operator, content: JSON.toJSONString(data, SerializerFeature.WriteMapNullValue, SerializerFeature.DisableCircularReferenceDetect))
        )
    }
}
